...
execute as @a run execute if score @s EMminedStone matches 1.. if predicate expertmodesilverfish:stonecheck at @s run tag @e[type=item,tag=!sfspawned,distance=0..10,limit=1,sort=nearest,nbt={Item:{id:"minecraft:cobblestone"}}] add sfspawner
...
execute as @a run execute if score @s EMminedStone matches 1.. run scoreboard players reset @s EMminedStone
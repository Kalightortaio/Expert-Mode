...
execute at @e[type=minecraft:leash_knot,tag=EHspawner] run playsound minecraft:entity.ender_eye.death block @e[type=player,distance=0..10]
kill @e[type=minecraft:leash_knot,tag=EHspawner]
execute as @a run execute store success score @s EMtpBool run execute if score @p EMtime matches 41 as @e[type=item] at @s run execute at @e[type=minecraft:armor_stand,tag=enderhopper,distance=0..4.5,sort=nearest,limit=1] run tp @e[type=item,distance=0..4.5] ~ ~0.5 ~
execute as @a run execute if score @s EMtpBool matches 1 at @e[type=minecraft:armor_stand,tag=enderhopper] run playsound minecraft:entity.enderman.teleport block @e[type=player,distance=0..10]
execute as @e[type=armor_stand,tag=enderhopper] at @s if block ~ ~ ~ minecraft:air run particle minecraft:smoke ~ ~ ~ 0.5 0.5 0.5 0 200
execute as @e[type=armor_stand,tag=enderhopper] at @s if block ~ ~ ~ minecraft:air run playsound minecraft:entity.ender_eye.death block @e[type=player,distance=0..10]
...
scoreboard players reset @a EMtpBool
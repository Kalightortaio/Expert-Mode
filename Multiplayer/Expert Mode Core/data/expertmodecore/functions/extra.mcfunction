...
execute as @a[gamemode=!creative] store success score @s EMhardelytra run clear @s minecraft:debug_stick 1
execute as @a[gamemode=!creative] if score @s EMhardelytra matches 1.. run give @s minecraft:elytra{AttributeModifiers:[{AttributeName:"generic.knockback_resistance",Name:"generic.knockback_resistance",Amount:0.1,Operation:0,UUID:[I;-1796047588,-1347663020,-1791671404,-174745534],Slot:"chest"},{AttributeName:"generic.armor_toughness",Name:"generic.armor_toughness",Amount:3,Operation:0,UUID:[I;-1011633566,805981030,-1351914856,1112753478],Slot:"chest"},{AttributeName:"generic.armor",Name:"generic.armor",Amount:8,Operation:0,UUID:[I;-1320329606,-1542765693,-1865745719,206807529],Slot:"chest"}]} 1
execute as @a[gamemode=!creative] if score @s EMhardelytra matches 1.. run scoreboard players reset @s EMhardelytra

# Experience Bottle Crafting
scoreboard players set @a EMvarOne 1
execute as @a[gamemode=!creative] if score @s EMexp matches 1.. run clear @s minecraft:experience_bottle 1
execute as @a[gamemode=!creative] if score @s EMexp matches 1.. run execute store result score @s EMexplevels run xp query @s levels
execute as @a[gamemode=!creative] if score @s EMexp matches 1.. run execute store result score @s EMexppoints run xp query @s points
execute as @a[gamemode=!creative] if score @s EMexp matches 1.. if score @s EMexplevels matches 0 if score @s EMexppoints matches 0..6 run effect give @s minecraft:instant_damage 1 0 true
execute as @a[gamemode=!creative] if score @s EMexp matches 1.. if score @s EMexplevels matches 0 if score @s EMexppoints matches 0..6 run give @s minecraft:glass_bottle 1
execute as @a[gamemode=!creative] if score @s EMexp matches 1.. if score @s EMexplevels matches 0 if score @s EMexppoints matches 0..6 run scoreboard players reset @s EMexp
execute as @a[gamemode=!creative] if score @s EMexp matches 1.. run xp add @s -7 points
execute as @a[gamemode=!creative] at @s if score @s EMexp matches 1.. run playsound minecraft:entity.evoker.cast_spell master @s ~ ~ ~ 0.5 0.3
execute as @a[gamemode=!creative] if score @s EMexp matches 1.. run give @s experience_bottle 1
execute as @a[gamemode=!creative] if score @s EMexp matches 1.. run scoreboard players operation @s EMexp -= @s EMvarOne
...
# execute at @a if block ~ ~-1 ~ minecraft:spawner{SpawnData:{ArmorItems:[{},{},{},{tag:{CustomModelData:1}}]}}
